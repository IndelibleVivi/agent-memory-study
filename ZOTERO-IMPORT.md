...
The expected result for current `main` is 16 top-level bibliographic items with 16 attachments:

- 9 stored PDFs copied into Zotero storage;
...
The RDF relies on relative paths for the nine bundled files, so importing a separately downloaded RDF without its adjacent `papers/` directory is incomplete.
