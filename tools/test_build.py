...
class PublicReadingRoomBuildTests(unittest.TestCase):
    ...
    def test_material_payload_cache_key_tracks_current_projection(self):
        ...
        self.assertEqual(
            source.count('assets/materials-data.js?v=20260925-representation-2'),
            1,
        )
    ...
    def test_stylesheet_cache_key_tracks_mobile_evidence_fix(self):
        source = (build.ROOT / "index.html").read_text(encoding="utf-8")
        self.assertEqual(
            source.count('assets/styles.css?v=20260925-representation-2'),
            1,
        )
    ...
    def test_existing_close_read_entries_keep_richer_read_schema(self):
        ...
        vermem = by_id["verifiable-memory"]
        self.assertEqual(vermem["noteDepth"], "worked")
        for field in (
            "whyRead", "argumentMap", "methodNotes", "reportedFindings", "evidenceLimits",
            "sourceTensions", "editorialInferences", "contributions",
        ):
            self.assertTrue(vermem[field])
        self.assertEqual(vermem["contributions"][0]["type"], "public-test")
        self.assertTrue(any(
            link["url"].endswith("/research/vermem-verifier-boundary-audit/results.json")
        ...
        proactive = by_id["proactive-wake-anchor"]
        self.assertEqual(proactive["noteDepth"], "worked")
        for field in (
            "whyRead", "argumentMap", "methodNotes", "reportedFindings", "evidenceLimits",
            "sourceTensions", "editorialInferences", "contributions",
        ):
            self.assertTrue(proactive[field])
        self.assertEqual(proactive["contributions"][0]["type"], "public-test")
        self.assertTrue(any(
            link["url"].endswith("/research/proactive-prefix-study/results.json")
    ...
    def test_practice_cache_keys_track_the_current_release(self):
        ...
        for asset in ("materials-data.js", "styles.css", "practice.js", "reading-search.js", "seo.js", "app.js"):
            with self.subTest(asset=asset):
                self.assertEqual(
                    source.count(f'assets/{asset}?v=20260925-representation-2'),
                    1,
                )
