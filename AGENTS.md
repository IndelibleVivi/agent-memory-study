...
- 新增小型 deterministic study 时接入 `tools/verify_reader.py`，同步 README / CONTRIBUTING 的验证范围；该入口校验 canonical / generated data、evidence、search 与已列入的五组本地研究，另检查 AgeMem 已保存 receipt。Receipt 校验不算 upstream 重跑；该入口不运行全部外部源码审计或论文 benchmark。
